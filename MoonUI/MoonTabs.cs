...
namespace MoonUI
{
    ...
    public class MoonTabs : TabControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            e.Graphics.Clear(MoonManager.BackAccentColor);
            ...
            if (TabCount > 0)
            {
                using (SolidBrush brushBackgroundTab = new SolidBrush(MoonManager.BackAccentColor))
                using (SolidBrush brushDivider = new SolidBrush(MoonManager.AccentColor))
                {
                    {
            ...
            using (Pen penActive = new Pen(MoonManager.AccentColor))
            using (Pen penBorder = new Pen(MoonManager.BackAccentColor, 0))
            using (SolidBrush brushActive = new SolidBrush(MoonManager.AccentColor))
            using (SolidBrush brushInActive = new SolidBrush(MoonManager.BackAccentColor))
            using (SolidBrush brushAlternative = new SolidBrush(MoonManager.AccentColor))
            using (SolidBrush brushActiveIndicator = new SolidBrush(ControlPaint.Light(MoonManager.AccentColor)))
            using (SolidBrush brushInActiveIndicator = new SolidBrush(MoonManager.AccentColor))
            using (brushActiveText = new SolidBrush(MoonManager.TextColor))
            using (SolidBrush brushInActiveText = new SolidBrush(MoonManager.TextColor))
            using (SolidBrush brushDrag = new SolidBrush(ControlPaint.Dark(MoonManager.AccentColor)))
            {
